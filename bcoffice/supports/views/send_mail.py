...
class SendMailView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        mongodb = BCOfficeHistoryMongoDB(collection=TBL_BCOFFICE_SEND_MAIL_HISTORY)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if title is None or contents is None :
            return Response(
                data=ResponseMessage.getMessageData(ResponseMessage.MESSAGE_ERR00017.format("메시지 또는 전화번호"))
                , status=status.HTTP_400_BAD_REQUEST)

        if send_type is not None and send_type == 'all':
            members = list(Members.objects.using('exchange').exclude(email__isnull = True).exclude(email__exact=''))

            for item in members:
                target_list.append(item.email)
        else :
        ...
        # MONGO DB 추가
        logging_utils.set_log(TBL_BCOFFICE_SEND_MAIL_HISTORY, log.toJsonString())
