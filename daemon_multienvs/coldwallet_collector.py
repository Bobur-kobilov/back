'''
usage: coldwallet_collector.py [-h] [--env ENV]

Coldwallet Collector

optional arguments:
  -h, --help  show this help message and exit
  --env ENV   environment: local, dev, stage, production
'''
...
supported_envs = [ 'local', 'dev', 'stage', 'production' ]
parser = argparse.ArgumentParser(description='Coldwallet Collector')
parser.add_argument('--env', help='environment: {}'.format(', '.join(supported_envs)))
args = parser.parse_args()

if args.env is None:    
    logger.error("Environment is required. Program exited.")
    sys.exit()
env = args.env.lower()
if env not in supported_envs:
    logger.error("{} is not supported enviroment.".format(env))
    sys.exit()

# To import config file by an environment
import importlib
config = importlib.import_module('config_{}'.format(env))    
    ...
    def save_currency_coin(self):
        ...
        for item in currency_list:
            ...
            for wallet in self.find_wallets(item['id']) :
                if coin_name in getBalanceOutsideList:
                    ...
                    if api is not None :
                        ...
                        if rs.status_code == 200:                            
                            ...
                            if coin_name == 'BTC':
                                ...
                                balance = data['Right']['caBalance']['getCoin']
                else:
                    manager = CoinManager(coin_manager_host = config.COIN_MANAGER_ADDRESS)
